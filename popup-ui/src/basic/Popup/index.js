...
class Popup extends Component {
    ...
    start({ ...config }) {
        this.setState({
            ...
            cancelButtonCallBack: config.callback !== undefined ? config.cancelButtonCallBack : this.defaultCallback(),
        })
    ...
    render() {
        const { title, type, textBody, button, buttonText, callback, background, cancelButtonText, cancelButtonCallBack } = this.state
        let el = null;
        let el2 = null;
        if (this.state.button) {
            el = <TouchableOpacity style={[styles.Button, styles[type]]} onPress={callback}>
                <Text numberOfLines={10} style={[styles.TextButton, type == 'Warning' ? { color: 'black' } : {}]}>{buttonText}</Text>
            </TouchableOpacity>
        }
        else {
            el = <Text></Text>
        ...
        if (this.state.cancelButtonText) {
            el2 = <TouchableOpacity style={[styles.Button, styles[type], { marginTop: 5 }]} onPress={cancelButtonCallBack ? cancelButtonCallBack : () => { }}>
                <Text numberOfLines={10} style={[styles.TextButton, type == 'Warning' ? { color: 'black' } : {}]}>{cancelButtonText}</Text>
            </TouchableOpacity>
        }
        else {
            el2 = <Text></Text>
            ...
            <Animated.View
                ...
                >
                    ...
                    <View style={styles.Content}>
                        ...
                        <Text style={styles.Desc} numberOfLines={15}>{textBody}</Text>
